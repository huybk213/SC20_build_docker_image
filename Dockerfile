...
ENV TZ=Asia/Kolkata
RUN apt update
RUN apt-get install -y git mkisofs bc python2.7 openjdk-8* gnupg flex bison gperf build-essential zip curl zlib1g-dev libc6-dev lib32ncurses5-dev x11proto-core-dev libx11-dev lib32z1-dev libgl1-mesa-dev g++-multilib tofrodos
...
RUN python -V
RUN mkdir /sc20
# Fix TLS error when running JACK server
